...
Matriz::Matriz(unsigned int linhas, unsigned int colunas, const double &valor=0){
    ...
    for(unsigned int i = 1; i < linhas; i++)
        for(unsigned int j = 1; j < colunas; j++)
            this->M[i][j] = valor;
}
...
Matriz::Matriz(const Matriz &m){
    ...
    for(unsigned int i = 1; i < _numero_de_linhas; i++)
        for(unsigned int j = 1; j < _numero_de_colunas; j++)
            this->M[i][j] = m.M[i][j];
}

Matriz::~Matriz(){
   for(unsigned int i = 1; i < this->_numero_de_linhas; ++i)
        delete [] this->M[i];
    delete [] this->M;
}
...
void Matriz::MatrizIdentidade(){
    for(unsigned int i = 1; i < this->_numero_de_linhas; i++){
        for(unsigned int j = 1; j < this->_numero_de_colunas; j++){
...
Matriz& Matriz::operator=( const Matriz &m ){
    for (unsigned int i = 1; i < m._numero_de_linhas; ++i)
...
ostream &operator<<(ostream &out, const Matriz &m ){
    for(unsigned int i = 1; i < m._numero_de_linhas; i++) {
        for(unsigned int j = 1; j < m._numero_de_colunas; j++)
            out << m.M[i][j] << "\t";
        out << endl;
    }
    return out;
